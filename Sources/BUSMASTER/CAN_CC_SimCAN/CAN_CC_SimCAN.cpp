...
/**
 * \file      CAN_CC_SimCAN.cpp
 * \brief     Source file for SimTecc SimCAN DIL functions
 * \author    Tobias Lorenz, Arunkumar Karri
 * \copyright Copyright (c) 2011, ETAS GmbH. All rights reserved.
 *
 * Source file for SimTecc SimCAN DIL functions
 */
// CAN_CC_SimCAN.cpp : Defines the initialization routines for the DLL.
//

/* C++ includes */

#include "CAN_CC_SimCAN_stdafx.h"
...
#include "EXTERNAL/inc/SimCAN.h"
#include "EXTERNAL/inc/simcan_public.h"
#include "EXTERNAL/inc/simcanerrorcodes.h"

#define USAGE_EXPORT
#include "CAN_CC_SimCAN_Extern.h"

BEGIN_MESSAGE_MAP(CCAN_CC_SimCAN, CWinApp)
END_MESSAGE_MAP()


/**
 * CCAN_CC_SimCAN construction
 */
CCAN_CC_SimCAN::CCAN_CC_SimCAN()
{
...
// The one and only CCAN_CC_SimCAN object
CCAN_CC_SimCAN theApp;


/**
 * CCAN_CC_SimCAN initialization
 */

static HINSTANCE ghLangInst = nullptr;

BOOL CCAN_CC_SimCAN::InitInstance()
{
...
HANDLE g_hDataEvent[MAX_CLIENT_ALLOWED] = { 0 };

static HWND sg_hOwnerWnd = nullptr;

/* CDIL_CAN_CCSimCAN class definition */
class CDIL_CAN_CCSimCAN : public CBaseDIL_CAN_Controller
{
public:
...
CDIL_CAN_CCSimCAN* g_pouDIL_CAN_CCSimCAN = nullptr;

/**
* \brief         Returns the CDIL_CAN_CCSimCAN object
* \param[out]    ppvInterface, is void pointer to take back the reference to CDIL_CAN_CCSimCAN object
...
USAGEMODE HRESULT GetIDIL_CAN_Controller(void** ppvInterface)
{
    ...
    if (nullptr == g_pouDIL_CAN_CCSimCAN)
    {
        if ((g_pouDIL_CAN_CCSimCAN = new CDIL_CAN_CCSimCAN) == nullptr)
        {
            hResult = S_FALSE;
        }
    }
    *ppvInterface = (void*)g_pouDIL_CAN_CCSimCAN;  /* Doesn't matter even if g_pouDIL_CAN_CCSimCAN is null */
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_SetAppParams(HWND hWndOwner)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_UnloadDriverLibrary(void)
{
    return S_OK;
}
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_ManageMsgBuf(BYTE bAction, DWORD ClientID, CBaseCANBufFSE* pBufObj)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_RegisterClient(BOOL bRegister, DWORD& ClientID, char* pacClientName)
{
    USES_CONVERSION;
    HRESULT hResult = S_FALSE;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetCntrlStatus(const HANDLE& /*hEvent*/, UINT& /*unCntrlStatus*/)
{
    return S_OK;
}
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_LoadDriverLibrary(void)
{
    return S_OK;
}
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_PerformInitOperations(void)
{
    HRESULT hResult = S_FALSE;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_PerformClosureOperations(void)
{
    HRESULT hResult = S_OK;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetTimeModeMapping(SYSTEMTIME& CurrSysTime, UINT64& TimeStamp, LARGE_INTEGER& QueryTickCount)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_ListHwInterfaces(INTERFACE_HW_LIST& asSelHwInterface, INT& nCount, PSCONTROLLER_DETAILS InitData)
{
    USES_CONVERSION;
    HRESULT hResult = S_FALSE;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_SelectHwInterface(const INTERFACE_HW_LIST& asSelHwInterface, INT /*nCount*/)
{
    USES_CONVERSION;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_DeselectHwInterface(void)
{
...
{
    return (g_pouDIL_CAN_CCSimCAN->CAN_SetConfigData(pDatStream, 0) == S_OK);
}
...
void CAN_setHardware(int nChannelCount)
{
    ...
    for (UINT nChannel = 0; nChannel < nChannelCount; nChannel++)
    {
        ...
        oss1 << "CC - " << sg_HardwareIntr[nChannel].m_acDescription.c_str() << " SN - " << sg_HardwareIntr[nChannel].m_dwVendor;
        ...
        sg_HardwareList.m_ouChannelDetails[nChannel].m_omVendorId = sg_HardwareIntr[nChannel].m_dwVendor;
        sg_HardwareList.m_ouChannelDetails[nChannel].m_omChannelName = sg_HardwareIntr[nChannel].m_acDescription.c_str();
        sg_HardwareList.m_ouChannelDetails[nChannel].m_omChannelIndex = nChannel;
    }
}
/**
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_SetConfigData(PSCONTROLLER_DETAILS ConfigFile, int /* Length */)
{
    VALIDATE_VALUE_RETURN_VAL(sg_bCurrState, STATE_HW_INTERFACE_SELECTED, ERR_IMPROPER_STATE);
...
/**
* \brief         This will convert the error code from CC SimCAN driver format
...
* \date          07.10.2011 Created
*/
static void ProcessCANMsg(UINT unClientIndex, const CanMsg& msg, DWORD dlc, CanFrameType frameType, const CanTimeStamp& timeStamp)
{
    sg_asCANMsg.m_lTickCount.QuadPart = (LONGLONG)((timeStamp.high << 32) | timeStamp.low);
    ...
    if (CREATE_MAP_TIMESTAMP == sg_byCurrState)
    {
        ...
        unConnectionTime = ((g_QueryTickCount.QuadPart * 10000) / sg_lnFrequency.QuadPart) - sg_TimeStamp;
        //Time difference should be +ve value
        if (sg_asCANMsg.m_lTickCount.QuadPart >= unConnectionTime)
        {
            sg_TimeStamp = (LONGLONG)(sg_asCANMsg.m_lTickCount.QuadPart - unConnectionTime);
        }
        else
        {
            sg_TimeStamp = (LONGLONG)(unConnectionTime - sg_asCANMsg.m_lTickCount.QuadPart);
        }
    ...
    if (dlc <= 8) /* Valid CAN message length */
    {
        /* Copy data length */
        sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_ucDataLen = (UCHAR)dlc;

        /* Copy the message data */
        memcpy(sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_ucData,
            msg.data, dlc);
    }

    /* Copy the message ID */
    sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_unMsgID = (UINT)msg.id;

    /* Check for extended message indication */
    sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_ucEXTENDED =
        (frameType & CAN_FRAME_EXTENDED) ? TRUE : FALSE;

    //if (sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_ucEXTENDED)
    //{
    //    sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_unMsgID ^= CAN_FRAME_EXTENDED;    // make it std
    //}

    /* Set channel ID */
    sg_asCANMsg.m_uDataInfo.m_sCANMsg.m_ucChannel = (UCHAR)unClientIndex + 1;
...
* \date          07.10.2011 Created
*/
DWORD WINAPI CanMsgReadThreadProc_CAN_CC_SimCAN(LPVOID pVoid)
{
    USES_CONVERSION;
    ...
    // TODO: For now we only handle CAN1
    g_hDataEvent[0] = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    ...
    while (bLoopON)
    {
        dwResult = WaitForSingleObject(pThreadParam->m_hActionEvent, 1); // If the event is not set wait 1ms instead of adding Sleep(1)
        ...
        case INVOKE_FUNCTION:
        {
            ...
            do
            {
                ...
                for (UINT i = 0; i < sg_nNoOfChannels; i++)
                {
                    ...
                    if (CanReceive(sg_aodChannels[i].m_hnd, &msg, &length, 0, CAN_FRAME_EXTENDED, &frameType, 0))
                    {
                        CanTimeStamp timeStamp;
                        CanGetLastTimeStamp(sg_aodChannels[i].m_hnd, &timeStamp);

                        ProcessCANMsg(i, msg, length, frameType, timeStamp);
                        bMoreDataExist = true;
                    }
                }
        ...
        {
            // TODO: No point having this case when vCreateTimeModeMapping is commented out?
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_StartHardware(void)
{
    VALIDATE_VALUE_RETURN_VAL(sg_bCurrState, STATE_HW_INTERFACE_SELECTED, ERR_IMPROPER_STATE);
    ...
    if (hResult == S_OK)
    {
        if (sg_sParmRThread.bStartThread(CanMsgReadThreadProc_CAN_CC_SimCAN))
        {
            hResult = S_OK;
        }
    }
...
static int nDisconnectFromDriver()
{
    ...
    for (UINT i = 0; i< sg_nNoOfChannels; i++)
    {
        if (!CanClose(sg_aodChannels[i].m_hnd))
        {
            nReturn = -1;
        }
    }
...
static int nConnect(bool bConnect)
{
    ...
    if (!sg_bIsConnected && bConnect) // Disconnected and to be connected
    {
        ...
        {
            TCHAR canNet[16] = { 0 };
            _stprintf(canNet, TEXT("CAN%d"), i + 1);

            sg_aodChannels[i].m_hnd = CanOpen(canNet);
            if (sg_aodChannels[i].m_hnd == 0)
            {
                success = FALSE;
                break;
            }
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_StopHardware(void)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetCurrStatus(STATUSMSG& StatusData)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetTxMsgBuffer(BYTE*& /*pouFlxTxMsgBuffer*/)
{
    return S_OK;
}
...
static int nWriteMessage(STCAN_MSG sMessage, DWORD dwClientID)
{
            ...
            (sg_nNoOfChannels <= defNO_OF_CHANNELS))
    {
        ...
        memcpy(msg.data, sMessage.m_ucData, sMessage.m_ucDataLen);

        DWORD length = sMessage.m_ucDataLen;

        if (CanSend(sg_aodChannels[sMessage.m_ucChannel - 1].m_hnd, &msg, length, FALSE))
        {
            nReturn = S_OK;
        }
    }
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_SendMsg(DWORD dwClientID, const STCAN_MSG& sMessage)
{
    VALIDATE_VALUE_RETURN_VAL(sg_bCurrState, STATE_CONNECTED, ERR_IMPROPER_STATE);
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetBusConfigInfo(BYTE* /*BusInfo*/)
{
    return S_OK;
}
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetLastErrorString(std::string& acErrorStr)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetControllerParams(LONG& lParam, UINT nChannel, ECONTR_PARAM eContrParam)
{
    HRESULT hResult = S_OK;
...
HRESULT CDIL_CAN_CCSimCAN::CAN_SetControllerParams(INT nValue, ECONTR_PARAM eContrparam)
{
    return S_OK;
}
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_CAN_CCSimCAN::CAN_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel, ECONTR_PARAM eContrParam)
{
    HRESULT hResult = S_OK;
...
static int nGetNoOfConnectedHardware(void)
{
    // TODO: Check simcan.xml how many nets are configured. For now assume CAN1 is always available.
...
unsigned int GetSelectedChannelIndex(unsigned int unIndex)
{
    ...
    for (int nChannel = 0; nChannel < defNO_OF_CHANNELS; nChannel++)
    {
        ...
        ossHardwaredesc << "CC - " << sg_HardwareList.m_ouChannelDetails[nChannel].m_omChannelName << ", SN - " << sg_HardwareList.m_ouChannelDetails[nChannel].m_omVendorId;
        ...
        else
        {
            ...
            ossHardwaredesc1 << "CC - " << sg_HardwareList.m_ouChannelDetails[nChannel].m_omChannelName << ", SN - " << sg_HardwareList.m_ouChannelDetails[nChannel].m_omVendorId;
...
static int nCreateMultipleHardwareNetwork(PSCONTROLLER_DETAILS InitData, UINT unDefaultChannelCnt = 0)
{
    ...
    for (unsigned int nCount = 0; nCount < nHwCount; nCount++)
    {
        sg_HardwareIntr[nChannels].m_dwIdInterface = nCount;
        sg_HardwareIntr[nChannels].m_dwVendor = 0; // N/A

        sg_HardwareIntr[nChannels].m_acDescription = "SimCAN";
        sg_HardwareIntr[nChannels].m_acDeviceName = "TBD";
    ...
    unDefaultChannelCnt = nChannels; // TODO: For now no selection dialog, only use default.
    ...
    for (int nCount = 0; nCount < sg_ucNoOfHardware; nCount++)
    {
        ...
        {
            sprintf(sg_aodChannels[nCount].m_strName, _("CC - %s, Serial Number- %ld, Firmware- %s"),
...
static int nCreateSingleHardwareNetwork()
{
    ...
    sg_HardwareIntr[0].m_acDescription = "SimCAN";
    sg_HardwareIntr[0].m_dwVendor = 0;
    sg_HardwareIntr[0].m_acDeviceName = "TBD";

    sprintf(sg_aodChannels[0].m_strName, _("CC - %s, Serial Number: %ld, Firmware: %s"),
...
static int nInitHwNetwork(PSCONTROLLER_DETAILS InitData, UINT unDefaultChannelCnt)
{
    ...
    sprintf(acNo_Of_Hw, _("Number of CC SimCan simulated CAN nets Available: %d"), nChannelCount);
...
HRESULT CDIL_CAN_CCSimCAN::CAN_SetHardwareChannel(PSCONTROLLER_DETAILS ouControllerDetails, DWORD /*dwDriverId*/, bool bIsHardwareListed, unsigned int unChannelCount)
{
